...
void InitialNavState::toInformation(
  ...
  information->setZero();
  Eigen::Vector3d positionVariance = sigma_p_WS.cwiseAbs2();
  Eigen::Vector3d orientationVariance = sigma_q_WS.cwiseAbs2();
...
void InitialNavState::toCovariance(
  ...
  covariance->setZero();
  covariance->diagonal().head<3>() = sigma_p_WS.cwiseAbs2();
...
std::string InitialNavState::toString() const {
  ...
  ss << "v_WS " << v_WS.transpose() << ", sigma v_WS " << sigma_v_WS.transpose()
     << ".\n";
...
void initBiasesFromStaticImu(const okvis::ImuMeasurementDeque &imuMeasurements,
  ...
  biases->measurement.accelerometers = accelSum * invnum + gravityB;
}
}  // namespace swift_vio
