...
void SimulatedMotionForParallaxAngleTest::dN_dp_WCmi(Eigen::Matrix3d* j) const {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    swift_vio::DirectionFromParallaxAngleJacobian dfpaj_delta(
...
void SimulatedMotionForParallaxAngleTest::simulateSidewaysMotion(
  ...
  swift_vio::removeUnsetMatrices<okvis::kinematics::Transformation>(
      &T_WC_list_, projectionStatus);
  swift_vio::removeUnsetMatrices<Eigen::Vector3d>(&observationsxy1_,
                                              projectionStatus);
  if (addOutlier) {
