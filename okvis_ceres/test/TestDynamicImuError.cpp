...
TEST(okvisTestSuite, DynamicImuError){
  ...
  OKVIS_DEFINE_EXCEPTION(Exception, std::runtime_error);
	...
	static_cast<DynamicImuErrorT*>(cost_function_imu)->
	...
	for(size_t i=0; i<6; ++i){
	  ...
	  //std::cout<<poseParameterBlock_0.estimate().T()<<std::endl;
		static_cast<DynamicImuErrorT*>(cost_function_imu)->Evaluate(parameters,residuals_p.data(),NULL);
	  ...
	  poseLocalParameterization->Plus(parameters[0],dp_0.data(),parameters[0]);
	  //std::cout<<poseParameterBlock_0.estimate().T()<<std::endl;
		static_cast<DynamicImuErrorT*>(cost_function_imu)->Evaluate(parameters,residuals_m.data(),NULL);
