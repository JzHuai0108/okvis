...
int predictStates(
  ...
  assert(imuMeasurements.front().timeStamp <= t_start &&
         imuMeasurements.back().timeStamp >= t_end);
  ...
  int i = 0;
  Scalar nexttime;
  for (typename GenericImuMeasurementDeque<Scalar>::const_iterator it =
    ...
    Eigen::Matrix<Scalar, 3, 1> vel_inc1 =
        (q_old.conjugate()._transformVector(a_est * dt) +
         q_new.conjugate()._transformVector(a_est * dt)) *
        Scalar(0.5);
...
int predictStatesBackward(
  ...
  assert(imuMeasurements.front().timeStamp <= t_end &&
         imuMeasurements.back().timeStamp >= t_start);
  ...
  int i = 0;
  Scalar nexttime;
  for (typename GenericImuMeasurementDeque<Scalar>::const_reverse_iterator it =
...
int predictStates(
  ...
  int i = 0;
  okvis::Time nexttime;
  for (typename okvis::ImuMeasurementDeque::const_iterator it =
           imuMeasurements.begin();
       it != imuMeasurements.end(); ++it) {
...
int predictStatesBackward(
  ...
  assert(imuMeasurements.front().timeStamp <= t_end &&
         imuMeasurements.back().timeStamp >= t_start);
  // if this assertion fails during optimization, it often means the time offset
  // variables diverge. Solution: either try to enlarge the range of imu reading
  // segment, or decrease the time variables' std to effectively lock them
  ...
  int i = 0;
  okvis::Time nexttime;
  for (typename okvis::ImuMeasurementDeque::const_reverse_iterator it =
           imuMeasurements.rbegin();
       it != imuMeasurements.rend(); ++it) {
